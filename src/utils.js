 ...
 * @param {int} index
 */
export function shouldDisplayLabel(length, index) {
  const minNumberLabels = 8
  const maxNumberLabels = 10
  const displayEvery = calculateDisplayEvery(
    minNumberLabels,
    maxNumberLabels,
    length,
  )
  return index % displayEvery === (length - 1) % displayEvery
}

function calculateDisplayEvery(min, max, length) {
  const every = length / max
  const lowerDivider = Math.floor(every) || 1
  const upperDivider = Math.ceil(every)

  if (length / lowerDivider <= max) {
    return lowerDivider
  }
